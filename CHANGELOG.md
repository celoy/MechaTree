...
### Added — Step 21c: tournament checkpoint/resume + parallel replicates runner + validation (2026-05-30)

- `mechatree.evolution.run.run_tournament` gained `resume_from: Path | None` parameter. When set, the run loads a prior `archive_XXXXXXXX.json` snapshot, reconstructs the survivor population, and continues from `generation + 1`. The returned `result.history` covers only the resumed portion (`[start_gen, n_generations)`); to reconstruct full history, concatenate prior archives with `result.history`. Forest layout does NOT resume (genomes only) — scientific accuracy maintained, bit-identical identity not guaranteed. Known limitation: resuming a multi-day run does not produce identical tree positions/ages as a fresh uninterrupted run (spatial layout is not serialized), but genomes and lineage identity are preserved.
- [`scripts/run_tournament.py`](scripts/run_tournament.py): new CLI for embarrassingly-parallel replicate tournament. Runs N independent Forest instances with different RNG seeds via `multiprocessing.Pool` (spawn context, macOS-compatible). Each replicate writes its own archive sequence to `out_dir/replicate_NN/`. After all replicates complete, survivors are combined (genomes + fitness) and curated for champions via `mechatree.evolution.curate._payload` → combined `champions.json`. CLI flags: `--config`, `--n-gens`, `--n-replicates`, `--archive-every`, `--resume-from` (applies same snapshot to all replicates), `--out-dir`, `--seed`, `--n-workers`, `--champions-out`, `--no-progress`.
- [`scripts/validate_tournament.py`](scripts/validate_tournament.py): validator comparing a Python champions JSON to the Fortran reference `data/S3_champions.json`. Required checks: (1) ≥1 species detected, (2) species centroid gap ≥ 0.15 (sufficiently separated). Optional checks: species count match Fortran (2), centroid L2 distance to reference < 0.30. Exit code 0 if required checks pass, 1 otherwise. CLI flags: `--python-champions`, `--reference`, `--gap-threshold`, `--centroid-distance-warn`.
- [`examples/tournament_natcomm.yaml`](examples/tournament_natcomm.yaml): tournament-scale configuration (R=200 L, n_trees_init=20000, n_trees_max=100000, n_generations=100000) reproducing Eloy et al. (2017, Nat Commun) island parameters. Fortran scale mapping: Ninit=10000 → Python n_trees_init=20000 (twig length physics), SizeForest=100 → Python size=200 (tree density ~0.16 trees/L²). Defaults: `cauchy=4.0e-5` (Fortran 2.0e-5 × 2 for explicit ½ρU²), `volume_ratio_leaf=4.0` (Nat Commun SI Fig. S12), `wind.model=default` (Fortran-faithful rotating storm), leaf_transparency=0.5, safety=3.0.
- [`tests/test_evolution_resume.py`](tests/test_evolution_resume.py): four tests validating resume functionality. Tests cover: (1) resumed run starts at correct generation, (2) history covers only resumed generations, (3) live forest exists after resume, (4) genome round-trip through dict (pickle-safe), (5) nonexistent snapshot path raises error.
- Performance context: island scale (R=200, n_trees_init=20000) reaches steady state at ~0.24 s/gen (`default` wind) or ~0.89 s/gen (`momentum` wind). A 100k-gen tournament runs 6.7–24.7 h single-threaded; N replicates in parallel reduces wall-clock by ~N (limited by shared resources on the machine).
